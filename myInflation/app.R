...
ui <- fluidPage(theme = shinytheme("cosmo"),
                ...
                navbarPage("MyInflation",
                           ...
                           sidebarLayout(
                             ...
                             sidebarPanel(
                               ...
                               conditionalPanel(
                                 ...
                                 textInput("incomeInput", "Annual Income in ($)", value = ""),
                                 conditionalPanel(
                                   condition = "input.incomeInput == ''",
                                   h5("ex. 36,000 or 36000")
                                 )
                               ...
                               conditionalPanel(
                                 condition = "input.tabSelected == 1 && input.advancedSet == 1",
                                 tags$b("Do You Buy These Items Frequently?")
                               ), 
                               conditionalPanel(
                                 condition = "input.tabSelected == 1 && input.advancedSet == 1",
                               ...
                               conditionalPanel(
                                 ...
                                 sliderInput("fatInput", "How Much do you Spend on Fats and Oils Monthly",
                                             min = 0, max = 150, value = 23)
                               ),
                               # Renting or Mortgage?
                               conditionalPanel(
                                 condition = "input.tabSelected == 1 && input.advancedSet == 1",

                                 selectInput("rentBuy", "Do you Rent, Own W/Mortgage, or Outright Own?", c("Renting", "Own w/Mortgage", "Own"))
                               ),
                               # Housing Expenses
                               conditionalPanel(
                                 condition = "input.tabSelected == 1",
                                 sliderInput("housingInput", "Your Monthly Housing Expenses: ",
                                             min = 0, max = 3000, value = 8200, sep = "")
                               ),
                               # Public or Private
                               conditionalPanel(
                                 condition = "input.tabSelected == 1 && input.advancedSet == 1",
                               ...
                               conditionalPanel(
                                 ...
                                 sliderInput("transInput", "Your Monthly Transportation Expenses: ",
                                             min = 0, max = 500, value = 3200, sep = "")
                               ),
                               # Medical
                               conditionalPanel(
                                 condition = "input.tabSelected == 1",
                                 sliderInput("medInput", "Your Monthly Medical Expenses: ",
                                             min = 0, max = 500, value = 480, sep = "")
                               ),
                         ...
                         tabPanel("Glossary",
                                  # defintions
                                  tags$h1("What is the difference between a general inflation rate and a personal inflation rate?"),
                                  tags$h5("A reasonable question is how does a personal inflation rate differ from the general inflation rate reported on in the newsmedia? To convey the difference, below are definitions of both a general inflation rate and a personal inflation rate."),
                                  tags$b("General Inflation Rate (or 'Inflation Rate'):"),
                                  br(),
                                  tags$h5("A general inflation rate or, as it is more often referred to -- an inflation rate -- is a calculation of the CPI-U by the United States Bureau of Labor Statistics (BLS). CPI-U refers to the CPI of Urban Consumers. Because most consumers in the United States live in regions which are considered to be 'Urban' by the BLS, the inflation rate calculated from the CPI-U is the inflation rate that is most referred to in the American newsmedia. However, that inflation rate is not necessarily indicative of all consumers."),
                                  tags$b("Personal Inflation Rate:"),
                                  tags$h5("A personal inflation rate is identical to a general inflation rate with the exception of how the market basket is built. In the calculation of a personal inflation rate, the market basket used is comprised of common expenses for the individual in question whereas a general inflation rate utilizes a market basket which is meant to be representative of a population's common expenses."),
                                  tags$h1("What use is a personal inflation rate?:"),
                                  tags$h5("As previously mentioned, the inflation rate most often reported on is derived from the CPI-U. However, that CPI in almost all cases, cannot be a perfect reflection of a single individual's common expenses and as such may not provide an accurate inflation rate for an individual. And, though there are a number of of alternative CPI's that can be used to find other inflation rates, all CPI's use a market basket which is a representation of a larger population and cannot reflect an individual's common expenses. As such, by determining some basic information about a person, including their geographic location of residence, income, and their common expenses, a personal inflation rate, tailored to an individual can be calculated."),
                                  tags$h1("General Definitions"),
                                  tags$b("Inflation: "),
                         ...
                         navbarMenu("More",
                                    tabPanel("Help", 
                                             tags$h1("How does myInflation work?"),
                                             br(),
                                             tags$h5("myInflation generates a personal inflation rate based upon information inputed by users. Primarily using geographic information, consumer buying habits, eating habits, housing situation, primary mode of transportation, medical expenses, school enrollment information and proportions of categorical spending."),
                                             tags$h1("How to use the tool"),
                                             tags$h5("When first using the myInflation tool, be sure to follow the following steps:"),
                                             tags$h5("1. Adjust the sliders to reflect your common expenses"),
                                             tags$h5("2. For a more advanced and accurate personal inflation rate, select the 'Advanced' box at the top of the panel and then input information as requested."),
                                             tags$h5("Using your inputs, myInflation takes data from the Bureau of Labor Statistics to create a personal inflation rate for you.")
                                             ),
                                    tabPanel("About", 
    ...
    output$personalRate <- renderUI({
      ...
      if (input$advancedSet == 1) {
        ...
        div (
          ...
          h5(paste("Percentage Change in Medical: ", percent((input$medInput - adjMedical) / adjMedical, accuracy = 0.01))),
          if (cpiEdu1 > 1) {
            h5(paste("Percentage Change in Education: ", percent((input$eduInput - adjEdu) / adjEdu, accuracy = 0.01)))
          },
          h5(paste("Percentage Change in Communications: ", percent((input$comInput - adjCell) / adjCell, accuracy = 0.01)))
          )
        #)
